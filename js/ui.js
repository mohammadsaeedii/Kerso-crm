/* ============================================================
   Kerso CRM — UI component library & behaviours
   Builder functions return HTML strings; controllers (DataTable,
   modal, drawer, popover, toast, tooltip) manage live behaviour.
   ============================================================ */
(function (App) {
  "use strict";
  const { escapeHtml, icon, fmt, node, qs, qsa, on } = App;
  ...
  function avatarGroup(people, max = 4, size = 28) {
    ...
    return (
      ...
      (extra > 0 ? `<span class="avatar avatar--more" style="width:${size}px;height:${size}px;font-size:${Math.round(size * 0.34)}px">+${extra}</span>` : "") +
      `</span>`
    );
  }

  /** Illustrated face avatar reused from the original Kerso design */
  ...
  const STATUS_MAP = {
    Active: "success", Won: "success", Customer: "success", positive: "success", open: "info",
    Lead: "info", Prospect: "indigo", Partner: "violet", neutral: "neutral",
    Churned: "danger", lost: "danger", negative: "danger", "Churn risk": "warning",
  };
  ...
  function tabs(items, active) {
    return (
      ...
      items
        .map(
          (t) =>
            `<button type="button" class="tab ${t.value === active ? "is-active" : ""}" data-tab="${escapeHtml(t.value)}" role="tab" aria-selected="${t.value === active}">${escapeHtml(t.label)}${t.count != null ? `<span class="tab__count">${t.count}</span>` : ""}</button>`
        )
        .join("") +
      `</div>`
    );
  ...
  function emptyState(opts) {
    return `<div class="empty">
      ...
      <p class="empty__title">${escapeHtml(opts.title || "Nothing here yet")}</p>
  ...
  function kpiCard(k, opts = {}) {
    return `<article class="card stat" data-kpi="${k.id}">
      <div class="stat__top">
        ...
        ${opts.menu !== false ? `<button class="stat__menu icon-btn icon-btn--sm" data-row-menu data-kpi="${k.id}" aria-label="Options">${icon("more-h", { size: 18 })}</button>` : ""}
      </div>
      <p class="card__value">${escapeHtml(k.display)}</p>
      <div class="stat__foot">
        <div class="card__foot">
          ${trendPill(k.delta, k.dir)}
          <span class="card__hint">vs last month</span>
        </div>
  ...
  function popover(anchor, html, opts = {}) {
    ...
    const reposition = () => {
      ...
      let left = opts.align === "end" ? r.right - ew : opts.align === "center" ? r.left + r.width / 2 - ew / 2 : r.left;
    ...
    // wrap close to also remove doc listener
    ...
    const root = node(`<div class="overlay" role="dialog" aria-modal="true">
      <div class="modal${sizeCls}" role="document">
        <header class="modal__head">
          ...
          <button class="icon-btn icon-btn--sm modal__close" aria-label="Close">${icon("x", { size: 20 })}</button>
        </header>
  ...
  function confirm(opts) {
    return new Promise((resolve) => {
      const footer = `<button class="btn btn--ghost" data-act="cancel">${escapeHtml(opts.cancelText || "Cancel")}</button>
        <button class="btn ${opts.danger ? "btn--danger" : "btn--primary"}" data-act="ok">${escapeHtml(opts.confirmText || "Confirm")}</button>`;
      const m = modal({
    ...
    const root = node(`<div class="overlay overlay--drawer" role="dialog" aria-modal="true">
      <aside class="drawer" style="${opts.width ? `width:${opts.width}px` : ""}">
        <header class="drawer__head">
          ...
          <button class="icon-btn icon-btn--sm drawer__close" aria-label="Close">${icon("x", { size: 20 })}</button>
        </header>
  ...
  function toast(message, opts = {}) {
    ...
    const t = node(`<div class="toast toast--${type}" role="status">
      ...
      <button class="toast__close" aria-label="Dismiss">${icon("x", { size: 16 })}</button>
    </div>`);
    host.appendChild(t);
  ...
  class DataTable {
        ...
        ? pageRows
            .map((row) => {
              ...
              return (
                ...
                (this.opts.rowActions ? `<td class="td-actions"><button class="icon-btn icon-btn--sm" data-row-action aria-label="Row actions">${icon("more-h", { size: 18 })}</button></td>` : "") +
                `</tr>`
              );
            })
            .join("")
        : `<tr><td class="td-empty" colspan="${this.columns.length + (sel ? 1 : 0) + (this.opts.rowActions ? 1 : 0)}">${emptyState({ icon: this.opts.emptyIcon || "search", title: this.opts.emptyTitle || "No results", desc: this.opts.emptyDesc || "Try adjusting your search or filters." })}</td></tr>`;

      const selCount = this.selected.size;
      const bulkBar = sel && selCount
        ? `<div class="table__bulk"><span class="table__bulk-count">${selCount} selected</span><div class="table__bulk-actions">${(this.opts.bulkActions || [])
            ...
            .join("")}<button class="btn btn--sm btn--ghost" data-bulk-clear>Clear</button></div></div>`
        : "";

      const showingTo = Math.min(start + this.pageSize, total);
      const pager = `<div class="table__foot">
        <span class="table__count">${total ? `Showing <b>${start + 1}–${showingTo}</b> of <b>${total}</b>` : "0 results"}</span>
        <div class="pagination">
          <button class="pg-btn" data-pg="prev" ${this.page === 1 ? "disabled" : ""} aria-label="Previous page">${icon("chevron-left", { size: 18 })}</button>
          ${this._pageButtons(pages)}
          <button class="pg-btn" data-pg="next" ${this.page === pages ? "disabled" : ""} aria-label="Next page">${icon("chevron-right", { size: 18 })}</button>
        </div>
      </div>`;
    ...
    _pageButtons(pages) {
      ...
      list.forEach((p) => {
        ...
        html += `<button class="pg-btn ${p === cur ? "is-active" : ""}" data-pg="${p}">${p}</button>`;
        prev = p;
      });
      return html;
    }
    _wire() {
      const c = this.c;
      if (this._wired) return; // delegate once
      this._wired = true;
