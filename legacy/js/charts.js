...
(function (App) {
    ...
    return (
      ...
      data
        .map((d, i) => {
          ...
          const color = d.color || PALETTE[i % PALETTE.length];
          const val = opts.money ? App.fmt.money(d.value) : opts.pct ? d.value + "%" : App.fmt.num(d.value);
      ...
      data
        .map((d, i) => {
          ...
          return `<div class="funnel__row" data-tip="${App.escapeHtml(d.stage)} — ${App.fmt.num(d.value)}">
            ...
            <div class="funnel__conv">${i === 0 ? "—" : conv.toFixed(0) + "% from previous"}</div>
          </div>`;
        })
        .join("") +
      `</div>`
  ...
  function heatmap(opts) {
    ...
    const head = `<div class="heat__row heat__row--head"><div class="heat__rh"></div>${App.range(cols)
      .map((c) => `<div class="heat__cell heat__cell--head">M${c}</div>`)
      .join("")}</div>`;
    const body = rows
      .map((row) => {
        const cells = App.range(cols)
          .map((c) => {
            ...
            return `<div class="heat__cell" style="background:rgba(79,70,229,${alpha.toFixed(2)});color:${v > 55 ? "#fff" : "var(--text)"}" data-tip="${row.label} · M${c}: ${v}%">${v}%</div>`;
          })
          .join("");
